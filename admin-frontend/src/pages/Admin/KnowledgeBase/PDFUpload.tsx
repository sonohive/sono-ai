...
export default function PDFUpload() {
  ...
  const fetchTrainingData = () => {
    setIsLoading(true);
    fetch(`/api/admin/knowledge?training_type=pdf&page=${page}&size=10`)
      .then(res => res.json())
      .then(json => {
        setData(json.items || []);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch('/api/admin/knowledge/ingest/pdf', {
        method: 'POST',
        body: formData
      });
      if (res.ok) {
        alert('PDF queued for ingestion successfully!');
        setSourceName('');
        setSourceUrl('');
        setCountry('');
        setFile(null);
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-3xl font-bold text-white tracking-tight flex items-center gap-3">
            <UploadCloud className="w-8 h-8 text-blue-400" /> Upload PDF
          </h1>
          <p className="text-slate-400 mt-1">Upload clinical PDFs to extract and add their content to the knowledge base.</p>
        </div>
      </div>
          ...
          <div>
            ...
            <div className="border-2 border-dashed border-white/10 rounded-xl p-8 flex flex-col items-center justify-center text-center bg-[#09090b] hover:bg-white/5 transition-colors">
              ...
              <label htmlFor="pdf-upload" className="cursor-pointer flex flex-col items-center">
                <div className="w-16 h-16 bg-white/5 border border-white/10 shadow-sm rounded-full flex items-center justify-center text-blue-400 mb-4">
                  <FileText className="w-8 h-8" />
                </div>
                <h3 className="text-lg font-bold text-white">
                  {file ? file.name : "Click to select a medical PDF"}
                </h3>
                <p className="text-sm text-slate-400 mt-2 max-w-md">
                  File will be uploaded to Cloudflare R2, embedded, and added to the Knowledge Base.
                </p>
              </label>
            </div>
          </div>
                  ...
                  data.map((item: any) => (
                    <tr key={item.id} className="hover:bg-white/[0.02] transition-colors">
                      ...
                      <td className="px-6 py-4 text-blue-400 hover:underline">
                        <a href={item.content_url} target="_blank" rel="noreferrer">View PDF</a>
                      </td>
