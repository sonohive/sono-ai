...
import { ArrowLeft, Save, Loader2, FileImage } from 'lucide-react';
...
export default function Media() {
  ...
  const fetchTrainingData = () => {
    setIsLoading(true);
    fetch(`/api/admin/knowledge?training_type=media&page=${page}&size=10`)
      .then(res => res.json())
      .then(json => {
        setData(json.items || []);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!file || !sourceName.trim()) return alert("File and Image Name/Description are required");
    ...
    try {
      const res = await fetch('/api/admin/knowledge/ingest/media', {
        method: 'POST',
        body: formData
      });
      if (res.ok) {
        alert('Media uploaded and embedded successfully!');
        setSourceName('');
        setSourceUrl('');
        setCountry('');
        setFile(null);
        ...
        alert(`Error: ${err.detail || 'Failed to upload'}`);
      }
    } catch (err) {
      console.error(err);
      alert('Network error');
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-3xl font-bold text-white tracking-tight flex items-center gap-3">
            <FileImage className="w-8 h-8 text-emerald-400" /> Upload Media
          </h1>
          <p className="text-slate-400 mt-1">Upload images with descriptive labels for multimodal queries.</p>
        </div>
      </div>
      ...
      <div className="bg-[#121214] border border-white/5 rounded-xl p-6">
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              <label className="block text-sm font-medium text-slate-400 mb-2">Image Name & Description</label>
              <input 
                type="text"
                value={sourceName}
                onChange={(e) => setSourceName(e.target.value)}
                placeholder="e.g. Fetal Ultrasound - 2nd Trimester"
                className={inputClass}
                required
              />
            </div>
          ...
          <div>
            <label className="block text-sm font-bold text-slate-300 mb-2">Upload Image File</label>
            <div className="border-2 border-dashed border-white/10 rounded-xl p-8 flex flex-col items-center justify-center text-center bg-[#09090b] hover:bg-white/5 transition-colors">
              ...
              <label htmlFor="image-upload" className="cursor-pointer flex flex-col items-center">
                <div className="w-16 h-16 bg-white/5 border border-white/10 shadow-sm rounded-full flex items-center justify-center text-emerald-400 mb-4">
                ...
                <p className="text-sm text-slate-400 mt-2 max-w-md">
                  Image will be uploaded to Cloudflare R2 and linked to the AI's training data.
                </p>
              </label>
            </div>
          </div>
                  ...
                  data.map((item: any) => (
                    <tr key={item.id} className="hover:bg-white/[0.02] transition-colors">
                      ...
                      <td className="px-6 py-4 text-blue-400 hover:underline">
                        <a href={item.content_url} target="_blank" rel="noreferrer">View Image</a>
                      </td>
